...
public class ZBufferRenderer extends TriangleRenderer {
			...
			if(Math.abs(renderPoints[1].coords[0]) > Math.abs(renderPoints[1].coords[1])) {
				...
				if(renderPoints[1].coords[0] >= 0) {
					for(int u=renderPoints[0].coords[0],du=0,v=renderPoints[0].coords[1],err=0;du <= renderPoints[1].coords[0];++u,++du) {
						...
						ZBufferRenderer.this.buf.set(u,v,renderPoints[0].coords[2] + (du * renderPoints[1].coords[2]) / renderPoints[1].coords[0], //
					...
					for(int u=renderPoints[0].coords[0],du=0,v=renderPoints[0].coords[1],err=0;du >= renderPoints[1].coords[0];--u,--du) {
						double f=du / (double)renderPoints[1].coords[0];
						buf.set(u,v,renderPoints[0].coords[2] + (du * renderPoints[1].coords[2]) / renderPoints[1].coords[0], //
								renderColors[0].coords[0] + f * renderColors[1].coords[0], //
								renderColors[0].coords[1] + f * renderColors[1].coords[1], //
								renderColors[0].coords[2] + f * renderColors[1].coords[2]);
						err-=renderPoints[1].coords[1];
				...
				if(renderPoints[1].coords[1] >= 0) {
					for(int v=renderPoints[0].coords[1],dv=0,u=renderPoints[0].coords[0],err=0;dv <= renderPoints[1].coords[1];++v,++dv) {
						...
						buf.set(u,v,renderPoints[0].coords[2] + (dv * renderPoints[1].coords[2]) / renderPoints[1].coords[1], //
								renderColors[0].coords[0] + f * renderColors[1].coords[0], //
								renderColors[0].coords[1] + f * renderColors[1].coords[1], //
								renderColors[0].coords[2] + f * renderColors[1].coords[2]);
						err+=renderPoints[1].coords[0];
	...
	public class RenderTriangleDelegate extends TriangleDelegate {
		...
		/**
		 * A triangle must be tesselated when its shortest side is shorter than THIN_THRESHOLD * its longest side.
		 */
		private final static double THIN_THRESHOLD=0.1;
		private final static int MAX_TESS_COUNT=10;
		@Override
		public void renderAsDelegate() {
			if(!normalsOK) { //could not calculate normals: triangle is flat.
				return;
			}
			...
			if(smin < THIN_THRESHOLD * smax) {
				tesselate(s0 < s1 ? s0 < s2 ? 0 : 2 : s1 < s2 ? 1 : 2,Math.min(MAX_TESS_COUNT,(int)Math.ceil(THIN_THRESHOLD * smax / smin)));
			} else {
				doRenderAsDelegate();
			}
		}
		...
		private void doRenderAsDelegate() {
			...
			int det=renderPoints[1].coords[0] * renderPoints[2].coords[1] - renderPoints[1].coords[1] * renderPoints[2].coords[0];
			...
			for(int u=minu;u <= maxu;++u) {
				for(int v=minv;v <= maxv;++v) {
					int l=sdet * ((u - renderPoints[0].coords[0]) * renderPoints[2].coords[1] - (v - renderPoints[0].coords[1]) * renderPoints[2].coords[0]);
					int m=sdet * (renderPoints[1].coords[0] * (v - renderPoints[0].coords[1]) - renderPoints[1].coords[1] * (u - renderPoints[0].coords[0]));
					if(l >= 0 && m >= 0 && l + m <= det) {
						//					if(u == buf.w / 2 && v == buf.h / 2) {
						//						System.err.println("" + u + "," + v + "," + (renderPoints[0].coords[2] + (l * renderPoints[1].coords[2] + m * renderPoints[2].coords[2]) / det));
						//					}
						buf.set(u,v,renderPoints[0].coords[2] + (l * renderPoints[1].coords[2] + m * renderPoints[2].coords[2]) / det,//
								renderColors[0].coords[0] + (l * renderColors[1].coords[0] + m * renderColors[2].coords[0]) / det,//
								renderColors[0].coords[1] + (l * renderColors[1].coords[1] + m * renderColors[2].coords[1]) / det,//
								renderColors[0].coords[2] + (l * renderColors[1].coords[2] + m * renderColors[2].coords[2]) / det);
					}
				}
			}
		}
