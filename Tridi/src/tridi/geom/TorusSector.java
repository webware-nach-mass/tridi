...
public class TorusSector implements Geometry {
	 ...
	 * sectors=0 means adaptative rendering
	 */
	public TorusSector(final SPoint center, final double minorRadius, final int minorSectors, final double majorRadius,
			final double angleStart, final double angleExtent, final int majorSectors) {
		super();
		this.center = center;
	...
	public void render(final String id, final TriangleRenderer renderer) {
		...
		if (ni == 0 || nj == 0) {
			...
			renderer.renderTriangle.points[1].set(center.coords[0] + majorRadius + minorRadius,
					center.coords[1] + majorRadius + minorRadius, center.coords[2] + minorRadius);
			...
			if (ni == 0) {
				ni = Math.max(6, (int) (0.5 * angleExtent * majorRadius * scale)); // each facet should be about 2px
																					// wide
			}
			if (nj == 0) {
		...
		for (int i = 0; i < ni; ++i) {
			...
			for (int j = 0; j < nj; ++j) {
				...
				renderer.renderTriangle.points[0].set(center.coords[0] + (majorRadius + minorRadius * u0) * c0,
						center.coords[1] + (majorRadius + minorRadius * u0) * s0, center.coords[2] + minorRadius * v0);
				renderer.renderTriangle.normals[0].set(u0 * c0, u0 * s0, v0);
				renderer.renderTriangle.points[1].set(center.coords[0] + (majorRadius + minorRadius * u1) * c0,
						center.coords[1] + (majorRadius + minorRadius * u1) * s0, center.coords[2] + minorRadius * v1);
				renderer.renderTriangle.normals[1].set(u1 * c0, u1 * s0, v1);
				renderer.renderTriangle.points[2].set(center.coords[0] + (majorRadius + minorRadius * u1) * c1,
						center.coords[1] + (majorRadius + minorRadius * u1) * s1, center.coords[2] + minorRadius * v1);
				...
				renderer.renderTriangle.points[0].set(center.coords[0] + (majorRadius + minorRadius * u0) * c0,
						center.coords[1] + (majorRadius + minorRadius * u0) * s0, center.coords[2] + minorRadius * v0);
				renderer.renderTriangle.normals[0].set(u0 * c0, u0 * s0, v0);
				renderer.renderTriangle.points[1].set(center.coords[0] + (majorRadius + minorRadius * u1) * c1,
						center.coords[1] + (majorRadius + minorRadius * u1) * s1, center.coords[2] + minorRadius * v1);
				renderer.renderTriangle.normals[1].set(u1 * c1, u1 * s1, v1);
				renderer.renderTriangle.points[2].set(center.coords[0] + (majorRadius + minorRadius * u0) * c1,
						center.coords[1] + (majorRadius + minorRadius * u0) * s1, center.coords[2] + minorRadius * v0);
		...
		for (int j = 0; j < nj; ++j) {
			...
			double u0 = Math.cos(b0), v0 = Math.sin(b0);
			double u1 = Math.cos(b1), v1 = Math.sin(b1);
			renderer.renderTriangle.points[0].set(center.coords[0] + majorRadius * c0,
					center.coords[1] + majorRadius * s0, center.coords[2]);
			renderer.renderTriangle.points[1].set(center.coords[0] + (majorRadius + minorRadius * u0) * c0,
					center.coords[1] + (majorRadius + minorRadius * u0) * s0, center.coords[2] + minorRadius * v0);
			renderer.renderTriangle.points[2].set(center.coords[0] + (majorRadius + minorRadius * u1) * c0,
					center.coords[1] + (majorRadius + minorRadius * u1) * s0, center.coords[2] + minorRadius * v1);
			renderer.renderTriangle.setSameNormals(s0, -c0, 0.0, true);
			renderer.renderTriangle.renderAsDelegate();
			renderer.renderTriangle.points[0].set(center.coords[0] + majorRadius * c1,
					center.coords[1] + majorRadius * s1, center.coords[2]);
			renderer.renderTriangle.points[1].set(center.coords[0] + (majorRadius + minorRadius * u0) * c1,
					center.coords[1] + (majorRadius + minorRadius * u0) * s1, center.coords[2] + minorRadius * v0);
			renderer.renderTriangle.points[2].set(center.coords[0] + (majorRadius + minorRadius * u1) * c1,
					center.coords[1] + (majorRadius + minorRadius * u1) * s1, center.coords[2] + minorRadius * v1);
	...
	public void renderObj(final String id, final ObjRenderer renderer) {
		...
		for (int i = 0; i < ni; ++i) {
			...
			for (int j = 0; j < nj; ++j) {
				...
				double u0 = Math.cos(b0), v0 = Math.sin(b0);
				double u1 = Math.cos(b1), v1 = Math.sin(b1);
				renderer.renderQuadrangle.points[0].set(center.coords[0] + (majorRadius + minorRadius * u0) * c0,
						center.coords[1] + (majorRadius + minorRadius * u0) * s0, center.coords[2] + minorRadius * v0);
				renderer.renderQuadrangle.normals[0].set(u0 * c0, u0 * s0, v0);
				renderer.renderQuadrangle.points[1].set(center.coords[0] + (majorRadius + minorRadius * u1) * c0,
						center.coords[1] + (majorRadius + minorRadius * u1) * s0, center.coords[2] + minorRadius * v1);
				renderer.renderQuadrangle.normals[1].set(u1 * c0, u1 * s0, v1);
				renderer.renderQuadrangle.points[2].set(center.coords[0] + (majorRadius + minorRadius * u1) * c1,
						center.coords[1] + (majorRadius + minorRadius * u1) * s1, center.coords[2] + minorRadius * v1);
				renderer.renderQuadrangle.normals[2].set(u1 * c1, u1 * s1, v1);
				renderer.renderQuadrangle.points[3].set(center.coords[0] + (majorRadius + minorRadius * u0) * c1,
						center.coords[1] + (majorRadius + minorRadius * u0) * s1, center.coords[2] + minorRadius * v0);
