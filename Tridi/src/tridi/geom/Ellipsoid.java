...
public class Ellipsoid implements Geometry {
	...
	public void render(final String id, final TriangleRenderer renderer) {
		...
		if (n == 0) {
			...
			n = Math.max(6, (int) (Math.PI
					* (Math.abs(size.coords[0]) + Math.abs(size.coords[1]) + Math.abs(size.coords[2])) / 6.0 * scale)); // each
																														// facet
																														// should
																														// be
																														// about
		...
		for (int i = 1; i < n; ++i) {
			...
			for (int j = -n; j < n; ++j) {
				...
				renderer.renderTriangle.points[0].set(center.coords[0] + radius.coords[0] * sla0 * clo0,
						center.coords[1] + radius.coords[1] * sla0 * slo0, z0);
				renderer.renderTriangle.normals[0].set(sla0 * clo0, sla0 * slo0, cla0);
				renderer.renderTriangle.points[1].set(center.coords[0] + radius.coords[0] * sla1 * clo1,
						center.coords[1] + radius.coords[1] * sla1 * slo1, z1);
				renderer.renderTriangle.normals[1].set(sla1 * clo1, sla1 * slo1, cla1);
				renderer.renderTriangle.points[2].set(center.coords[0] + radius.coords[0] * sla0 * clo2,
						center.coords[1] + radius.coords[1] * sla0 * slo2, z0);
		...
		for (int i = 1; i < n; ++i) {
			...
			for (int j = -n; j < n; ++j) {
				double lo0 = j * Math.PI / n, lo1 = (j + 0.5) * Math.PI / n, lo2 = (j + 1) * Math.PI / n, //
						...
						clo1 = Math.cos(lo1), slo1 = Math.sin(lo1), //
						clo2 = Math.cos(lo2), slo2 = Math.sin(lo2);
				renderer.renderTriangle.points[0].set(center.coords[0] + radius.coords[0] * sla1 * clo0,
						center.coords[1] + radius.coords[1] * sla1 * slo0, z1);
				renderer.renderTriangle.normals[0].set(sla1 * clo0, sla1 * slo0, cla1);
				renderer.renderTriangle.points[1].set(center.coords[0] + radius.coords[0] * sla0 * clo1,
						center.coords[1] + radius.coords[1] * sla0 * slo1, z0);
				renderer.renderTriangle.normals[1].set(sla0 * clo1, sla0 * slo1, cla0);
				renderer.renderTriangle.points[2].set(center.coords[0] + radius.coords[0] * sla1 * clo2,
						center.coords[1] + radius.coords[1] * sla1 * slo2, z1);
