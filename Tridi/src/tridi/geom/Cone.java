...
public class Cone implements Geometry {
	 ...
	 * sectors=0 means adaptative rendering.
	 */
	public Cone(final SPoint baseCenter,final double baseRadius,final double topRadius,final int sectors,final double length) {
		super();
	...
	public void render(final String id,final TriangleRenderer renderer) {
		...
		if(n == 0) {
			...
			n=Math.max(6,(int)(Math.PI * Math.max(baseRadius,topRadius) * scale)); //each facet should be about 2px wide
		}
		...
		for(int i=0;i < n;++i) {
			...
			renderer.renderTriangle.points[0].setFrom(baseCenter);
			renderer.renderTriangle.points[1].set(baseCenter.coords[0] + dx0b,baseCenter.coords[1] + dy0b,baseCenter.coords[2]);
			renderer.renderTriangle.points[2].set(baseCenter.coords[0] + dx1b,baseCenter.coords[1] + dy1b,baseCenter.coords[2]);
			renderer.renderTriangle.setSameNormals(0.0,0.0,-1.0,true);
			renderer.renderTriangle.renderAsDelegate();
			renderer.renderTriangle.points[0].set(baseCenter.coords[0] + dx1b,baseCenter.coords[1] + dy1b,baseCenter.coords[2]);
			renderer.renderTriangle.normals[0].set(c1 * txy,s1 * txy,s1 * tz);
			renderer.renderTriangle.points[1].set(baseCenter.coords[0] + dx0b,baseCenter.coords[1] + dy0b,baseCenter.coords[2]);
			renderer.renderTriangle.normals[1].set(c0 * txy,s0 * txy,s0 * tz);
			renderer.renderTriangle.points[2].set(baseCenter.coords[0] + dx0t,baseCenter.coords[1] + dy0t,baseCenter.coords[2] + length);
			renderer.renderTriangle.normals[2].set(c0 * txy,s0 * txy,s0 * tz);
			renderer.renderTriangle.normalsOK=true;
			renderer.renderTriangle.renderAsDelegate();
			renderer.renderTriangle.points[0].set(baseCenter.coords[0] + dx1b,baseCenter.coords[1] + dy1b,baseCenter.coords[2]);
			renderer.renderTriangle.normals[0].set(c1 * txy,s1 * txy,s1 * tz);
			renderer.renderTriangle.points[1].set(baseCenter.coords[0] + dx0t,baseCenter.coords[1] + dy0t,baseCenter.coords[2] + length);
			renderer.renderTriangle.normals[1].set(c0 * txy,s0 * txy,s0 * tz);
			renderer.renderTriangle.points[2].set(baseCenter.coords[0] + dx1t,baseCenter.coords[1] + dy1t,baseCenter.coords[2] + length);
			...
			renderer.renderTriangle.points[1].set(baseCenter.coords[0] + dx1t,baseCenter.coords[1] + dy1t,baseCenter.coords[2] + length);
			renderer.renderTriangle.points[2].set(baseCenter.coords[0] + dx0t,baseCenter.coords[1] + dy0t,baseCenter.coords[2] + length);
			renderer.renderTriangle.setSameNormals(0.0,0.0,1.0,true);
			renderer.renderTriangle.renderAsDelegate();
		}
	...
	public void renderObj(final String id,final ObjRenderer renderer) {
		...
		for(int i=0;i < n;++i) {
			...
			renderer.renderTriangle.setSameNormals(0.0,0.0,-1.0,true);
			renderer.renderTriangle.renderAsDelegate();
			renderer.renderQuadrangle.points[0].set(baseCenter.coords[0] + dx1b,baseCenter.coords[1] + dy1b,baseCenter.coords[2]);
			renderer.renderQuadrangle.normals[0].set(c1 * txy,s1 * txy,s1 * tz);
			renderer.renderQuadrangle.points[1].set(baseCenter.coords[0] + dx0b,baseCenter.coords[1] + dy0b,baseCenter.coords[2]);
			renderer.renderQuadrangle.normals[1].set(c0 * txy,s0 * txy,s0 * tz);
			renderer.renderQuadrangle.points[2].set(baseCenter.coords[0] + dx0t,baseCenter.coords[1] + dy0t,baseCenter.coords[2] + length);
			renderer.renderQuadrangle.normals[2].set(c0 * txy,s0 * txy,s0 * tz);
			renderer.renderQuadrangle.points[3].set(baseCenter.coords[0] + dx1t,baseCenter.coords[1] + dy1t,baseCenter.coords[2] + length);
