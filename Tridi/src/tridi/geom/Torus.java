...
public class Torus implements Geometry {
	 ...
	 * sectors=0 means adaptative rendering
	 */
	public Torus(final SPoint center, final double minorRadius, final int minorSectors, final double majorRadius,
			final int majorSectors) {
		super();
		this.center = center;
	...
	public void render(final String id, final TriangleRenderer renderer) {
		...
		if (ni == 0 || nj == 0) {
			...
			if (ni == 0) {
				ni = Math.max(6, (int) (Math.PI * majorRadius * scale)); // each facet should be about 2px wide
			}
			if (nj == 0) {
		...
		for (int i = 0; i < ni; ++i) {
			double a0 = -Math.PI + (i * 2.0 * Math.PI) / ni, a1 = -Math.PI + ((i + 1) * 2.0 * Math.PI) / ni;
